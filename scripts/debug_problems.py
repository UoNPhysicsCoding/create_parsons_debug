...
def create_debug_problem_html(title, instructions, initial_lines, vartests, output_name):
    ...
    # Feedback links and debug feedback container: remove for unit-tests-only outputs
    if with_unittests:
        problem_html = problem_html.replace("FEEDBACK_LINKS", "")
        problem_html = problem_html.replace("DEBUG_FEEDBACK_CONTAINER", "")
    else:
        feedback_block = ("<p>\n        <a href=\"#\" id=\"newInstanceLink\">New instance</a>\n"
                      "        <a href=\"#\" id=\"feedbackLink\">Get feedback</a>\n"
                      "    </p>")
        problem_html = problem_html.replace("FEEDBACK_LINKS", feedback_block)
        problem_html = problem_html.replace("DEBUG_FEEDBACK_CONTAINER", "<div id=\"debug-feedback\"></div>")
